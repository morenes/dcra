...
void RouterModel::comp_destXY(int qid, uint16_t orig, u_int32_t msg_data, int & dst_X, int & dst_Y, bool & route_to_core, bool & exact_x, bool & exact_y, bool & switch_X_board, bool & switch_Y_board){
   ...
   #if PROXY_FACTOR>1
      // FOR QID=3, ROUTING T3' MESSAGES!! i.e., T2 explicitly spawned a T3'
      if (qid == 3){
         ...
         int y_within = dst_region_Y * PROXY_H + offset_in_rep_Y;
            ...
            dst_Y = dst_Y % GRID_Y;
         #else
            // dst_X and dst_Y is updated for routing purposes, but the message's X/Y coordinates are not altered
            dst_X = x_within;
            dst_Y = y_within;
         #endif
               ...
               if (origin < DIR_NONE){
                  ...
                  bool t3_to_core = false;
                     ...
                     if (t3_to_core){
                        ...
                        owner_core = (dst_X == id_X && dst_Y == id_Y) && (!output_q[C][2].cannot_fit_two()); // It's the owner can can fit the message
                        // If it's not the owner or the owner is full, then check the IQ of T3'
